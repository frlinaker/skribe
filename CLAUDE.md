...
pytest --cov=promptlearn --cov-report=html     # with coverage

# Lint / format
black promptlearn/ tests/
ruff check promptlearn/ tests/
isort promptlearn/ tests/
mypy promptlearn/
...
Tests in `conftest.py` set `PROMPTLEARN_MODEL=gpt-5.4-mini` so CI uses a cheap model. Tests that make live LLM calls require an API key (`OPENAI_API_KEY` by default).
...
```
BasePromptEstimator (base.py)          ← all LLM interaction + code generation lives here
├── PromptClassifier (classifier.py)   ← thin wrapper, classification prompt template
├── PromptRegressor (regressor.py)     ← thin wrapper, regression prompt template
└── PromptFeatureEngineer              ← generates transform(**features) -> dict
    └── AdaptiveFeatureEngineer        ← skips FE if probe CV shows no improvement
```

`BasePromptEstimator` inherits `sklearn.base.BaseEstimator` so all scikit-learn protocols (Pipeline, GridSearchCV, clone, joblib) work automatically.
...
`PromptFeatureEngineer.transform()` validates that every row returns the same dict keys; inconsistency triggers a retry at fit time.

### LLM routing (`base._call_llm`)

Model selection: explicit `model=` arg > `PROMPTLEARN_MODEL` env var > `DEFAULT_MODEL` (`"gpt-5.5"`).
