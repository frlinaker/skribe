...
class BaseSkribeEstimator(BaseEstimator):
    ...
    def _build_dataset_context(
        ...
        prompt = (
            ...
            "5. CRITICAL — do not invent semantics. Only state what a column or coded value means "
            "if the raw description (or the column name itself) actually says so. Do not guess a "
            "meaning from a dataset's name, from general/outside knowledge of a similarly-named "
            "dataset, or from the shape/cardinality of the values, even if you recognize the "
            "dataset — anonymized or re-versioned copies of well-known datasets do not necessarily "
            "preserve the original category encodings. If the description gives no real semantics "
            "for a column (e.g. bare 'V1'..'V8' with no explanation of what the codes mean), say so "
            "explicitly, e.g. '- v1: meaning not stated in source data; treat as an unordered "
            "nominal category, do not assume what the codes represent.' The same applies to the "
            "target column and its value codes.\n\n"
